...
export function useValidatedForm<T extends Record<string, unknown>>(
  ...
  const errors = Object.keys(form.formState.errors).reduce(
    (acc, key) => {
      const error = form.formState.errors[key as keyof T];
      acc[key as keyof T] = error?.message as string | undefined;
      return acc;
    },
...
export function getFieldProps<T extends Record<string, unknown>>(
  ...
  return {
    ...
    isInvalid: !!form.formState.errors[fieldName],
  };
}
...
export function getFieldStatus<T extends Record<string, unknown>>(
  ...
  if (dirtyFields[fieldName]) return 'valid';
  return 'idle';
}
